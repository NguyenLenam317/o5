...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  function getDeviceIdFromRequest(req: Request): string {
    // Extract device ID from header or generate a fallback
    const deviceId = req.header('X-Device-ID') || `fallback-${Date.now()}`;
    return deviceId;
  }
  ...
  app.get("/api/user/profile", async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        id: userId,
        username: user?.username || `device_${deviceId.substring(0, 8)}`,
        hasSurveyCompleted,
        userProfile,
        deviceId
      });
  ...
  app.post("/api/user/profile", async (req: Request, res: Response) => {
    try {
      ...
      // Get or create user associated with this device
      ...
      if (!user) {
        ...
        user = await storage.createUser({
          ...
          password: "not_used", // Password not required for device users
          deviceId: deviceId
        });
      }
  ...
  app.post("/api/user/survey/complete", async (req: Request, res: Response) => {
    try {
      ...
      await storage.updateSurvey(user.id, {
        completed: true,
        lastStep: lastStep || 0
      });
